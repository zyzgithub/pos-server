...
public class JeecgOneToMainUtil {
	...
	public static void main(String[] args) {
		...
		codeParamEntityIn.setTableName("T_FZ_WFPROCESS");//主表[表名]
		codeParamEntityIn.setEntityName("Process");	 //主表[实体名]
		codeParamEntityIn.setEntityPackage("process");	 //主表[包名]
		codeParamEntityIn.setFtlDescription("工作流程");	 //主表[描述]
		
		//第二步：设置子表集合
		List<SubTableEntity> subTabParamIn = new ArrayList<SubTableEntity>();
		//[1].子表一
		SubTableEntity po = new SubTableEntity();
		po.setTableName("T_FZ_WFACTIVITY");//子表[表名]
		po.setEntityName("Activity");	 //子表[实体名]
		po.setEntityPackage("process");			 //子表[包]
		po.setFtlDescription("流程节点");		 //子表[描述]
		//子表[外键:与主表关联外键]
		//说明：这里面的外键是子表的外键字段,非主表和子表的对应关系
		po.setForeignKeys(new String[]{"processid"});
