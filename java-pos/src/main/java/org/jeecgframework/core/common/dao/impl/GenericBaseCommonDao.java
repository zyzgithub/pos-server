...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	...
	public <T> List<T> findObjForJdbc(String sql, int page, int rows,Class<T> clazz, Object... objs) {
		...
		//封装分页SQL
		sql = JdbcDao.jeecgCreatePageSql(sql,page,rows);
		List<Map<String, Object>> mapList = this.jdbcTemplate.queryForList(sql,objs);
		T po = null;
		for(Map<String,Object> m:mapList){
			try {
				po = clazz.newInstance();
	...
	public Long insertBySql(final String sql, final Object... param) {
	...
	public <T> T findOneForJdbc(String sql, Class<T> clz, Object... objs) {
		try{ 
			return this.jdbcTemplate.queryForObject(sql, clz , objs);
		}catch (EmptyResultDataAccessException e) {   
		    return null;   
		}  
	}
	...
	@Override
	public <T> List<T> findObjForJdbc(String sql, Class<T> clazz, Object... args) {
//		return this.jdbcTemplate.queryForList(sql, clazz, args);
	...
	public <T> List<T>  findForJdbcParam(String  sql,  int page, int rows, Class<T> clazz ,Object... objs){
		//封装分页SQL
		sql = JdbcDao.jeecgCreatePageSql(sql,page,rows);	
