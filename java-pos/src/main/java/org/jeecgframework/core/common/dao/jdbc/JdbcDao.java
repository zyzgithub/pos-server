...
public class JdbcDao extends SimpleJdbcTemplate{
	 ...
	 * 按照数据库类型，封装SQL
	 * 如果传入的page小于0，默认设置为1
	 * 如果传入的rows小于0，默认设置为10
	 */
	public static String jeecgCreatePageSql(String sql, int page, int rows){
		if(page <= 0){
			page = 1;//如果传入的page小于0，默认设置为1
		}
		if(rows <= 0){
			rows = 10;//如果传入的rows小于0，默认设置为10
		}
		int beginNum = (page - 1) * rows;
		
		String beginNumStr = String.valueOf(beginNum);
		String rowsStr = String.valueOf(rows);
		if(dataBaseType.indexOf(DATABSE_TYPE_MYSQL)!=-1){
			sql = MessageFormat.format(MYSQL_SQL, sql, beginNumStr, rowsStr);
		}else if(dataBaseType.indexOf(DATABSE_TYPE_POSTGRE)!=-1){
			sql = MessageFormat.format(POSTGRE_SQL, sql, beginNumStr, rowsStr);
		}else if(dataBaseType.indexOf(DATABSE_TYPE_ORACLE)!=-1){
			int beginIndex = (page-1)*rows;
			int endIndex = beginIndex+rows;
			sql = MessageFormat.format(ORACLE_SQL, sql, String.valueOf(beginIndex), String.valueOf(endIndex));
		}
		return sql;
	}
}
