...
public class UserController {
	...
	@RequestMapping(params = "existUser")
	@ResponseBody
	public JSONObject existUser(String mobile) {
		boolean b = userService.existUser(mobile);
		
		JSONObject response = new JSONObject();
		response.put("code", b ? 0 : 1);
		response.put("msg", b ? "该手机号已注册" : "该手机号可注册");
	...
	@RequestMapping(params = "userListByDepart")
	public String userListByDepart(HttpServletRequest request) {
		if(StringUtil.isNotEmpty(request.getParameter("isframe"))){
			request.setAttribute("isframe", request.getParameter("isframe"));
		}
		if(StringUtil.isNotEmpty(request.getParameter("search"))){
			request.setAttribute("search", request.getParameter("search"));
		}
		if(StringUtil.isNotEmpty(request.getParameter("checkbox"))){
			request.setAttribute("checkbox",request.getParameter("checkbox"));
		}
		return "system/user/userListByDepart";
		//return "system/user/userList-search-demo";
	}
	/**
	 * 用户信息
...
//				user.setPassword(PasswordUtil.encrypt(user.getUserName(), newpassword, PasswordUtil.getStaticSalt()));
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		String departname = oConvertUtils.getString(request.getParameter("TSDepart.departname"));
		if (!StringUtil.isEmpty(departname)) {
			cq.createAlias("TSDepart", "TSDepart");
			cq.eq("TSDepart.departname", departname);
			cq.add();
...
//		cq.add();
	...
	@RequestMapping(params = "checkUserMobilePhone")
	@ResponseBody
	public ValidForm checkUserMobilePhone(HttpServletRequest request) {
		ValidForm v = new ValidForm();
		String mobilePhone=oConvertUtils.getString(request.getParameter("param"));
		String code=oConvertUtils.getString(request.getParameter("code"));
		List<TSUser> roles=systemService.findByProperty(TSUser.class,"mobilePhone",mobilePhone);
		if(roles.size()>0&&!code.equals(mobilePhone))
		{
			v.setInfo("电话号码已存在");
			v.setStatus("n");
		}
...
//				user.setPassword(PasswordUtil.encrypt(user.getUserName(), password, PasswordUtil.getStaticSalt()));
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
		...
		jeecg.system.pojo.base.TSTypegroup typegroup = systemService.getTypeGroupByCode("area");
		String[] area = null;
	...
	@RequestMapping(params = "datagridUserByDepart")
	public void datagridUserByDepart(TSUser tuser,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid,String projectId,String roleType) {
		String departid = request.getParameter("departid");
		CriteriaQuery cq = new CriteriaQuery(TSBaseUser.class, dataGrid);
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tuser);

		cq.createAlias("TSDepart", "TSDepart",JoinType.LEFT_OUTER_JOIN);
		...
		if (departid != null && departid.length() > 0) {
			...
			userVO = systemService.get(TSUser.class, userVO.getId());
			cq.eq("TSDepart.id",userVO.getTSDepart().getId());
		}
		cq.add();

		if(StringUtil.isNotEmpty(tuser.getTSDepart().getDepartname())){
			if(StringUtil.strPos(tuser.getTSDepart().getDepartname(), "*")){
				String str = StringUtil.stringReplace(tuser.getTSDepart().getDepartname(), "\\*", "%");
				cq.like("TSDepart.departname", str);
			}else{
				cq.eq("TSDepart.departname", tuser.getTSDepart().getDepartname());
			}
		}
		
		this.systemService.getDataGridReturn(cq, true);
		List<TSBaseUser> users = dataGrid.getReaults();
		TagUtil.datagrid(response, dataGrid);
	}
