...
<script type="text/javascript">
	...
	$(function() {
		$('#functionid').tree({
			checkbox : true,
			url : basePath+'/roleController.do?setAuthority&roleId=${roleId}',
			...
			onClick: function(node){
				...
				if(isRoot==''){
					var roleId = $("#rid").val();
					$('#operationListpanel').panel("refresh", basePath+"/roleController.do?operationListForFunction&functionId="+node.id+"&roleId="+roleId);
				}else {
				}
			}
		});
	...
	function mysubmit() {
		var roleId = $("#rid").val();
		var s = GetNode();
		doSubmit(basePath+"/roleController.do?updateAuthority&rolefunctions=" + s + "&roleId=" + roleId);
	}
	function GetNode() {
